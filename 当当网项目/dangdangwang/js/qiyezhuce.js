var dj=document.querySelector('.submits');
var	input=document.querySelectorAll('.he');
var shu=document.querySelectorAll('.shu-ru');
console.log(shu)
console.log(input)
console.log(dj);
var kong=document.querySelectorAll('form .kong');
console.log(kong);
console.log(kong.length);
dj.onclick=function(){
	for(var i=0;i<kong.length;i++){
		kong[i].style.display='block';
	}
}
for(var i=0;i<input.length;i++){
	...
	input[i].onfocus=function(){
		for(var j=0;j<shu.length;j++){
			...
			kong[j].style.display='none';

		}
		this.style.display='block';
		shu[this.index].style.display="block"
		input[this.index].style.border="1px solid #000"
	}
}
// dengluyanzhengma画布
...
var shuaxin=document.querySelector('.shuaxin');
	...
	//绘制四个随机的字符
		...
		for(var i=0;i<10;i++){
			ctx.beginPath();//开始
			ctx.moveTo(randNum(0,115),randNum(0,40));
			ctx.lineTo(randNum(0,115),randNum(0,40));
		...
		for(var i=0;i<10;i++){
			ctx.beginPath();//开始
			ctx.arc(randNum(0,115),randNum(0,40),randNum(0,5),0,2*Math.PI);
			ctx.strokeStyle=randColor(60,160);
			ctx.stroke();//轮廓线
		}
	}	
